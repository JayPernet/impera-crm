...
interface ProcedimentosMaisRealizadosCardProps {
  organizationId: string | null;
}

export function ProcedimentosMaisRealizadosCard({ organizationId }: ProcedimentosMaisRealizadosCardProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      // Get completed appointments from last 30 days grouped by procedure
  ...
  return (
    <Card className="glass-card">
      <CardHeader className="pb-2">
        ...
        <p className="text-xs text-muted-foreground">Últimos 30 dias (concluídos)</p>
      </CardHeader>
      <CardContent className="space-y-3">
        {procedures.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-4">
            Nenhum procedimento concluído no período
          </p>
        ) : (
